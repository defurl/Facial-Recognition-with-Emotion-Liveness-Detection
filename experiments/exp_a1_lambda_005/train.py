#!/usr/bin/env python3
"""
Experiment A1: TDA Regularization with λ=0.05, No Warmup

Based on PATH_FORWARD.md Option A1:
- Lower TDA lambda: 0.05 (instead of 0.10)
- No warmup: TDA regularization from epoch 1
- Cosine annealing with warm restarts for learning rate
- Extended training: 30 epochs with early stopping

Benchmark results showed λ=0.05 gave:
- Verification AUC: 93.78% (vs 89.46% baseline TDA)
- Need more epochs to potentially improve classification accuracy

Usage:
    conda run -n face_recog python experiments/exp_a1_lambda_005/train.py
"""
...
# ============================================================================
EXPERIMENT_NAME = "exp_a1_lambda_005"
...
# TDA Regularization - OPTIMIZED VALUES
TDA_LAMBDA = 0.05           # Lower λ for better embedding quality
...
def train_epoch(model, dataloader, triplet_loss, tda_loss, optimizer, device, 
    ...
    # No warmup - use full lambda from epoch 1
...
def main():
    ...
    log("=" * 70)
    log(f"TDA Regularization with λ={TDA_LAMBDA}, No Warmup")
