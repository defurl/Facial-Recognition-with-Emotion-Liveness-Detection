#!/usr/bin/env python3
"""
Experiment A2: TDA Regularization with λ=0.02, Extended Training

Based on PATH_FORWARD.md Option A:
- Very low TDA lambda: 0.02 (minimal regularization, focus on embedding quality)
- No warmup: TDA regularization from epoch 1
- Cosine annealing with warm restarts for learning rate
- Extended training: 50 epochs with early stopping patience 15

Hypothesis: Even lower λ may produce better verification embeddings
while maintaining classification accuracy.

Usage:
    conda run -n face_recog python experiments/exp_a2_lambda_002/train.py
"""
...
# ============================================================================
EXPERIMENT_NAME = "exp_a2_lambda_002"
EXPERIMENT_DIR = Path(__file__).parent
OUTPUT_DIR = EXPERIMENT_DIR / "outputs"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

# Training hyperparameters - EXTENDED TRAINING
NUM_EPOCHS = 50
EMBEDDING_DIM = 256
EARLY_STOPPING_PATIENCE = 15

# TDA Regularization - VERY LOW LAMBDA
TDA_LAMBDA = 0.02           # Very low λ for maximum embedding focus
TDA_WARMUP_EPOCHS = 0       # No warmup - TDA from epoch 1
TDA_CACHE_PATH = PROJECT_ROOT / "outputs" / "tda_cache" / "tda_train.npz"
...
def load_tda_cache(cache_path, image_paths):
    ...
    if not cache_path.exists():
        log(f"WARNING: TDA cache not found at {cache_path}")
        return None, None
    
    log(f"Loading TDA cache from {cache_path}...")
    ...
    log(f"Matched {found}/{len(image_paths)} training images to TDA cache")
...
def main():
    ...
    log("=" * 70)
    log(f"TDA Regularization with λ={TDA_LAMBDA}, Extended Training")
    ...
    log(f"  TDA warmup epochs: {TDA_WARMUP_EPOCHS}")
    log(f"  Device: {DEVICE}")
    ...
    # Optimizer with cosine annealing (longer period for extended training)
    ...
    log(f"TDA regularization: {'Enabled' if use_tda else 'Disabled (no cache)'}")
    log("-" * 70)
    ...
    log(f"  TDA Regularized (λ=0.10): 94.45%")
    log(f"  Exp A1 (λ=0.05):      94.16%")
    log(f"  This experiment:      {best_val_acc*100:.2f}%")
    improvement = best_val_acc - 0.9415
    if improvement > 0:
        log(f"  ✓ IMPROVEMENT over baseline: +{improvement*100:.2f}%")
    else:
        log(f"  ✗ No improvement over baseline: {improvement*100:.2f}%")
