#!/usr/bin/env python3
"""
Experiment A3: TDA Regularization with λ=0.03 (Fine-tuned)

Based on results from A1 (λ=0.05) and A2 (λ=0.02):
- A1 achieved best results with λ=0.05
- A2 peaked too early with λ=0.02
- A3 tries λ=0.03 as interpolation point

Configuration:
- TDA Lambda: 0.03 (between A1 and A2)
- No warmup: TDA regularization from epoch 1
- Cosine annealing with warm restarts
- 30 epochs with early stopping patience 12

Usage:
    conda run -n face_recog python experiments/exp_a3_lambda_003/train.py
"""
...
# ============================================================================
EXPERIMENT_NAME = "exp_a3_lambda_003"
...
NUM_EPOCHS = 30
EMBEDDING_DIM = 256
EARLY_STOPPING_PATIENCE = 12  # Slightly more patience

# TDA Regularization - FINE-TUNED VALUES
TDA_LAMBDA = 0.03           # Interpolated between A1 (0.05) and A2 (0.02)
...
def main():
    ...
    log("=" * 70)
    log(f"TDA Regularization with λ={TDA_LAMBDA} (Fine-tuned)")
    ...
    log(f"  Exp A1 (λ=0.05):      94.54%")
    log(f"  Exp A2 (λ=0.02):      94.42%")
