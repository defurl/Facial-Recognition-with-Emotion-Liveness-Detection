...
def face_roles(face_idx, current_face_id, primary_face_id, enable_multi_face):
...
def role_box_color(is_primary_face, is_secondary_face):
    ...
    def capture_frames(self):
        ...
        while self.running:
            ...
            for face_idx, (x, y, w, h) in enumerate(faces_to_process):
                ...
                # Determine processing type and color coding
                is_primary_face, is_secondary_face = face_roles(
                    face_idx,
                    current_face_id,
                    self.primary_face_id,
                    self.ENABLE_MULTI_FACE_VERIFICATION,
                )
                ...
                # CRITICAL FIX: Only process primary face for full identity verification
                    ...
                    if cropped_face.size > 0 and cropped_face.shape[0] >= 50:
                        ...
                        if is_primary_face and cropped_face.size > 0:
                            try:
                                early_identity = self.liveness_adapter.compute_early_identity(
                                ...
                                if early_identity and self.frame_count % 15 == 0:
                        ...
                        if (is_primary_face and self.emotion_analysis_enabled and 
                            ...
                            try:
                                ...
                                cache_hit_identity = self.liveness_adapter.pick_cached_identity(
                                    early_identity,
                                    self.session_spoof_passed,
                                    employee_db,
                                )
                                if cache_hit_identity and self.frame_count % 30 == 0:
